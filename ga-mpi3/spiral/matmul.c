		...
		int blockX, int blockY) 
{
	...
	double *a, *b, *c[2]={NULL,NULL}, *atrans;
	...
	MPI_Alloc_mem(blockX * blockY * sizeof(double), MPI_INFO_NULL, &c[0]);
	MPI_Alloc_mem(blockX * blockY * sizeof(double), MPI_INFO_NULL, &c[1]);
...
#endif
	/* Ditto for C and B */
	//status = _ga_create(MPI_COMM_WORLD, dims[0], dims[1], MPI_DOUBLE, &g_b); 
	//status = _ga_create(MPI_COMM_WORLD, dims[0], dims[1], MPI_DOUBLE, &g_c); 
			...
			for (int k = 0; k < K; k+=blockX) {
				...
				for (int n = 0; n < N; n+=blockY) {
...
#endif
					if (dbl_bufcnt > 1) MPI_Wait(&reqs[dbl_bufcnt % 2], MPI_STATUS_IGNORE);
					...
					cblas_dgemm ( CblasRowMajor, CblasNoTrans, /* TransA */CblasNoTrans, /* TransB */
							...
							beta, c[dbl_bufcnt % 2], blockX /* ldc */);
					/* C = m x n */
					lo[0] = m; lo[1] = n;
					hi[0] = blockX + lo[0]; hi[1] = blockY + lo[1];				
					hi[0] = hi[0]-1; hi[1] = hi[1]-1;
...
#endif
					_nga_nbacc(g_c, lo, hi, c[dbl_bufcnt % 2], ld, &reqs[dbl_bufcnt % 2]);
					dbl_bufcnt++;
				} /* END LOOP N */
				next_gac = _ga_read_inc(g_cnt, rdcnt, (long)1);
			} /* ENDIF if count == next */
