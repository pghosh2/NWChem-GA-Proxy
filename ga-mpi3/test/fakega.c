...
{ 
	return _ga_transfer (ga, lo[0], lo[1], hi[0], hi[1], buf, ld, PUT);
} 

int _nga_get( GA ga, int * lo, int * hi, void * buf, int * ld ) 
{ 
	return _ga_transfer (ga, lo[0], lo[1], hi[0], hi[1], buf, ld, GET);
} 

int _nga_acc( GA ga, int * lo, int * hi, void * buf, int * ld ) 
{ 
	return _ga_transfer (ga, lo[0], lo[1], hi[0], hi[1], buf, ld, ACC);
} 
...
long _ga_read_inc( GA ga, int * subscript, long inc ) 
{
        ...
        long otemp = 0; /* will contain value of remote counter _prior_ to increment */
